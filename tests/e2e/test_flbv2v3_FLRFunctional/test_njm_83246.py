r"""NJM-83246 — [FLB v1] FLB - OS Support - Verify End-to-End Workflow (via FLR recovery to CIFS
share) on SLES 15.
...
pytestmark = [pytest.mark.flb, pytest.mark.flrfunctional, pytest.mark.jira("NJM-83246")]

MACHINE = "SLE_SUSE_15.205"
MANIFEST = "manifest-sles15-mixed.sha256"


@allure.title("NJM-83246 — OS-support E2E via FLR to CIFS on SLES 15")
def test_os_support_flr_e2e(logged_in_page, flb_job_cleanup):
    page = logged_in_page
    job_name = flb_job_cleanup("AUTO_FLB_NJM-83246")
    ...
    assert status == "Successful", f"job did not succeed on SLES 15: {status}"

    started = recover_to_share(page, job_name, ["root", "TestData_ForFLB"], ["MixedTypes"], "cifs")
    assert started, "the FLR wizard did not confirm the recovery started on SLES 15"
