r"""NJM-83252 — [FLB v1] FLB - OS Support - Verify End-to-End Workflow (via FLR recovery to CIFS
share) on AlmaLinux 9.
...
pytestmark = [pytest.mark.flb, pytest.mark.flrfunctional, pytest.mark.jira("NJM-83252")]

MACHINE = "AlmaLinux9_16.48"
MANIFEST = "manifest-almalinux9-mixed.sha256"


@allure.title("NJM-83252 — OS-support E2E via FLR to CIFS on AlmaLinux 9")
def test_os_support_flr_e2e(logged_in_page, flb_job_cleanup):
    page = logged_in_page
    job_name = flb_job_cleanup("AUTO_FLB_NJM-83252")
    ...
    assert status == "Successful", f"job did not succeed on AlmaLinux 9: {status}"

    started = recover_to_share(page, job_name, ["root", "TestData_ForFLB"], ["MixedTypes"], "cifs")
    assert started, "the FLR wizard did not confirm the recovery started on AlmaLinux 9"
