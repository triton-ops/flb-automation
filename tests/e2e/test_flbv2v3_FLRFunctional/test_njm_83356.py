"""NJM-83356 — [FLB v1] FLR from FLB - Functional - Verify Recovery from an Amazon S3 Repository.

Per the TC's own Xray steps: confirm an FLB job targeting an Amazon S3 repository has a valid
recovery point; open Recover > Individual files; on the Files step select items to recover; on
the Options step choose Recover to CIFS Share (or NFS Share), finish and run; on the destination
share, verify recovered files match the source content.

Uses Amazon_Repo (test-data/environment.md: id 9, type AWS S3, state OK — the plain,
non-immutable S3 repository on nbr-84; confirmed present live 2026-07-23 via the wizard's own
Destination-step repo search).

⚠ COST NOTE: this row uploads real data to a real external AWS S3 account. Run deliberately, not
by default in a routine batch pass.
...
pytestmark = [pytest.mark.flb, pytest.mark.flrfunctional, pytest.mark.jira("NJM-83356")]

MACHINE = "Window11"
REPOSITORY = "Amazon_Repo"


@allure.title("NJM-83356 — recovery from an Amazon S3 repository, recover to CIFS Share")
def test_recover_from_amazon_s3_repository(logged_in_page, flb_job_cleanup):
    page = logged_in_page
    job_name = flb_job_cleanup("AUTO_FLB_NJM-83356")
