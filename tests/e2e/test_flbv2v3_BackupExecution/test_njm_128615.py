r"""NJM-128615 — [FLB v3] FLB - Functional (Retention) - Verify Active Full After Source File
Deletion.

TWO-PHASE COMBINATION TEST — see test_njm_185029.py's module docstring for the full procedure
rationale. Uses the dedicated `ChangeTest_ForFLB/active_after_deletion` fixture folder (seeded
2026-07-20) and `full_backup_mode='Active full'` + 'Job runs #'/every_job_runs=1.

⚠ Same scope note as test_retention_full_backup_mode.py: no UI marker distinguishes an
Active-full recovery point from a Synthetic-full one — this verifies the setting is accepted and
that the new full backup genuinely excludes the deleted file, not the internal mechanism.

Procedure:
  1. `pytest test_njm_128615.py::test_baseline` — builds+runs once. No flb_job_cleanup — the job
     must survive into phase 2.
  2. Agent runs WinRM to delete `delete_me.txt` under `active_after_deletion`.
  3. `pytest test_njm_128615.py::test_after_source_deletion` — reruns the SAME job and asserts
...
pytestmark = [pytest.mark.flb, pytest.mark.backupexecution, pytest.mark.jira("NJM-128615")]

MACHINE = "Window11"
JOB_NAME = "AUTO_FLB_NJM-128615_active-full-deletion"
DRILL_TO_PARENT = ["Local Disk (C:)", "TestData_ForFLB", "ChangeTest_ForFLB"]
FLR_DRILL_TO_FOLDER = ["C:", "TestData_ForFLB", "ChangeTest_ForFLB", "active_after_deletion"]
BASELINE_FILES = {"keep.txt", "modify_me.txt", "delete_me.txt"}
AFTER_DELETION_FILES = BASELINE_FILES - {"delete_me.txt"}


@allure.title("NJM-128615 phase 1/2 — Active-full baseline run")
def test_baseline(logged_in_page):
    page = logged_in_page
    build_flb_job(
        page, JOB_NAME, MACHINE, DRILL_TO_PARENT, ["active_after_deletion"],
...
@allure.title("NJM-128615 phase 2/2 — Active-full rerun after a real WinRM-driven source deletion")
...
def test_after_source_deletion(logged_in_page, flb_job_cleanup):
    ...
    assert names == AFTER_DELETION_FILES, (
        f"expected delete_me.txt excluded from the new Active full, got {names}"
    )
