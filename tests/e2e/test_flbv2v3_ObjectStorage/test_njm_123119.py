r"""NJM-123119 — [FLB v1] FLB - Functional - Verify Backup to the Azure Blob Repository (Windows &
Linux Sources).
...
pytestmark = [pytest.mark.flb, pytest.mark.objectstorage, pytest.mark.jira("NJM-123119")]

REPOSITORY = "Azure_Repo"


@allure.title("NJM-123119 — backup to Azure Blob repository (Windows), full + incremental + checksum verify")
def test_backup_to_azure_windows(logged_in_page, flb_job_cleanup):
    page = logged_in_page
    job_name = flb_job_cleanup("AUTO_FLB_NJM-123119_azure-win")
...
@allure.title("NJM-123119 — backup to Azure Blob repository (Linux), full + incremental + checksum verify")
def test_backup_to_azure_linux(logged_in_page, flb_job_cleanup):
    page = logged_in_page
    job_name = flb_job_cleanup("AUTO_FLB_NJM-123119_azure-linux")
