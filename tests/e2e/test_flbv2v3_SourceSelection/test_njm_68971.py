r"""NJM-68971 — [FLB v2] FLB - Functional - Verify Backup/Recovery of Non-ASCII (Unicode)
Filenames: Cyrillic файл.txt, CJK 测试文件.docx, Arabic ملف.pdf.

Backs up the whole Windows folder containing the special file(s) (so the picker's own display
never has to type/round-trip the unicode name), runs the job, opens FLR, selects the target
...
pytestmark = [
    pytest.mark.flb, pytest.mark.sourceselection, pytest.mark.jira("NJM-68971"),
...
@allure.title("NJM-68971 — backup/recovery of unicode filenames (Windows), recover to CIFS")
def test_special_file_backup_recovery(logged_in_page, flb_job_cleanup):
    page = logged_in_page
    job_name = flb_job_cleanup("AUTO_FLB_NJM-68971")
    ...
    started = recover_to_share(
        page, job_name, ["C:", FIXTURE_FOLDER], ["файл.txt", "测试文件.docx", "ملف.pdf"], "cifs"
    )
