r"""NJM-68981 — [FLB v2] FLB - Functional - Verify Backup/Recovery of Various File Extensions:
...
pytestmark = [
    pytest.mark.flb, pytest.mark.sourceselection, pytest.mark.jira("NJM-68981"),
...
@allure.title("NJM-68981 — backup/recovery of various extensions (.iso/.exe), recover to CIFS")
def test_special_file_backup_recovery(logged_in_page, flb_job_cleanup):
    page = logged_in_page
    job_name = flb_job_cleanup("AUTO_FLB_NJM-68981")
    ...
    started = recover_to_share(
        page, job_name, ["C:", FIXTURE_FOLDER], ["sample.iso", "sample.exe"], "cifs"
    )
