r"""NJM-68982 — [FLB v2] FLB - Functional - Verify Backup/Recovery of Files Without an Extension:
READMEnoext, Makefile.

Backs up the whole Windows folder containing the special file(s), runs the job, opens FLR, selects
the target file(s), and recovers them to a CIFS share.
...
pytestmark = [
    pytest.mark.flb, pytest.mark.sourceselection, pytest.mark.jira("NJM-68982"),
...
@allure.title("NJM-68982 — backup/recovery of extensionless files (Windows), recover to CIFS")
def test_special_file_backup_recovery(logged_in_page, flb_job_cleanup):
    page = logged_in_page
    job_name = flb_job_cleanup("AUTO_FLB_NJM-68982")
    ...
    started = recover_to_share(
        page, job_name, ["C:", FIXTURE_FOLDER], ["READMEnoext", "Makefile"], "cifs"
    )
