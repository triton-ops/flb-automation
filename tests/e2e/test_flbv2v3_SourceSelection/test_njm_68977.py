r"""NJM-68977 — [FLB v2] FLB - Functional - Verify Backup/Recovery of Hidden Files: hidden_attr.txt
(+h attribute).

Backs up the whole Windows folder containing the special file(s) (so the hidden-attribute file is
in scope regardless of picker display), runs the job, opens FLR, selects the target file, and
recovers it to a CIFS share.
...
pytestmark = [
    pytest.mark.flb, pytest.mark.sourceselection, pytest.mark.jira("NJM-68977"),
...
@allure.title("NJM-68977 — backup/recovery of hidden files (Windows), recover to CIFS")
def test_special_file_backup_recovery(logged_in_page, flb_job_cleanup):
    page = logged_in_page
    job_name = flb_job_cleanup("AUTO_FLB_NJM-68977")
    ...
    started = recover_to_share(
        page, job_name, ["C:", FIXTURE_FOLDER], ["hidden_attr.txt"], "cifs"
    )
