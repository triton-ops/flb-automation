r"""NJM-68979 — [FLB v2] FLB - Functional - Verify Backup/Recovery of Shortcut Files (.lnk):
shortcut.lnk.

Backs up the whole Windows folder containing the special file(s), runs the job, opens FLR, selects
the target file, and recovers it to a CIFS share.
...
pytestmark = [
    pytest.mark.flb, pytest.mark.sourceselection, pytest.mark.jira("NJM-68979"),
...
@allure.title("NJM-68979 — backup/recovery of shortcut files (.lnk), recover to CIFS")
def test_special_file_backup_recovery(logged_in_page, flb_job_cleanup):
    page = logged_in_page
    job_name = flb_job_cleanup("AUTO_FLB_NJM-68979")
    ...
    started = recover_to_share(
        page, job_name, ["C:", FIXTURE_FOLDER], ["shortcut.lnk"], "cifs"
    )
