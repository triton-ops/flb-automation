r"""NJM-68978 — [FLB v2] FLB - Functional - Verify Backup/Recovery of Common Archive Files:
...
pytestmark = [
    pytest.mark.flb, pytest.mark.sourceselection, pytest.mark.jira("NJM-68978"),
...
@allure.title("NJM-68978 — backup/recovery of archive files (.zip/.rar), recover to CIFS")
def test_special_file_backup_recovery(logged_in_page, flb_job_cleanup):
    page = logged_in_page
    job_name = flb_job_cleanup("AUTO_FLB_NJM-68978")
    ...
    started = recover_to_share(
        page, job_name, ["C:", FIXTURE_FOLDER], ["sample.zip", "sample.rar"], "cifs"
    )
