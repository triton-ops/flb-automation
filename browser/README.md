...
browser/
  ...
  pom/
    ...
    backup_types/     # one page object per specific backup job type/flow
      ...
      backup_copy_recovery_page.py    # for a Backup Copy job's target — provides both
                                       # recover_file_level() (FLB source) and recover_file_share() (FSB source)
...
| Backup Copy Recovery | select a Backup Copy job → Recover | ✅ **POM calibrated + `check_backup_copy_recovery.py` dry-run ALL PASS** (live-verified 2026-07-08): `BackupCopyRecoveryPage` provides both `recover_file_level()` (for FLB-sourced copies) and `recover_file_share()` (for FSB-sourced copies), with comprehensive docstrings explaining when to use each. The Recover menu is the SAME shared 'GRANULAR RECOVERY' menu every job type uses, with items enabled/disabled by the copied backup's ORIGINAL source type, never the job's own fixed `hvType:"VMWARE"`. Built two temporary calibration jobs copying an FLB backup — confirmed 'File level recovery' enabled, 'File share recovery' present-but-disabled, and `FileLevelRecoveryManagement.createSession` with `hvType:"PHYSICAL"` (ORIGINAL source, not `VMWARE`) mounts + browses successfully. ⚠ Freshly-completed copy's savepoint can be `isAccessible:false` on some repos (observed on Cloudian; Recover greyed out too) but `isAccessible:true` on local repos — check target repo health first. |
