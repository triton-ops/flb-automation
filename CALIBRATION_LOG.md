...
| **Linux FLR tree root node** | **A Linux source's FLR left tree top-level node is `"root"`, not the wizard drill path — hit independently 7 times across the Inventory suite** | `test_njm_67702.py`, `test_njm_67806.py`–`test_njm_67809.py`, `test_njm_67813.py`, `test_njm_67816.py`, `test_njm_67817.py`, `test_njm_68933.py`, `test_njm_68934.py` |
...
| Area | Finding | Ref |
|---|---|---|
| **Login post-submit wait** | `wait_idle()` (`page.wait_for_load_state("networkidle")`) reliably timed out (20s) as the appliance's own Activities/Issues load grew — replaced with a deterministic wait for the URL to actually leave the login page | `login_page.py` |
| **Encryption password dialog — the toggle** | The 'Create password:' toggle is `<input type="button" role="radio">`, NOT `<input type="radio">` — a literal `input[type='radio']` search finds ZERO elements (an earlier attempt wrongly concluded no real radio input exists at all). It's also rendered off-screen (bounding box x≈-9491, the visible circle is drawn by CSS on a sibling), so any coordinate-based Playwright click (plain or `force=True`) always fails with "element is outside of the viewport" — only `Locator.evaluate("el => el.click()")` reliably flips it | `locators.py` (`OptionsLocators.ENCRYPTION_CREATE_PASSWORD_RADIO`), `flb_wizard_page.py` (`set_encryption_password()`) |
| **Encryption password dialog — required field** | The dialog's `Description:` textarea is REQUIRED for Create-password submission — leaving it empty puts a red validation border on it and the submit click silently no-ops (dialog stays open); nothing in the visible label text hints at this | `locators.py` (`OptionsLocators.ENCRYPTION_DESCRIPTION_TEXTAREA`), `flb_wizard_page.py` (`set_encryption_password()`) |
| **KMS-disabled confirmation dialog on Finish (the costly one)** | With a password configured but the Key Management Service disabled, clicking Finish/Finish & Run pops a SEPARATE confirmation modal ("Key Management Service is Disabled") on top of the wizard, distinct from the 'Set a password' dialog above. Left undismissed, the wizard's page-covering mask never clears and the job is NEVER actually created — confirmed live: 120s of polling never cleared the mask, and the job was absent from the sidebar afterward. Looks exactly like a hung backend save; it's really just an un-clicked Proceed button | `locators.py` (`OptionsLocators.KMS_DISABLED_DIALOG_TITLE`/`KMS_DISABLED_DIALOG_PROCEED`), `flb_wizard_page.py` (`_dismiss_kms_warning_if_present()`, called from `finish()`/`finish_and_run()`) |
| **`finish()` mask wait** | An encrypted job's Finish click leaves a page-covering `x-mask` up for well over the old fixed 2s wait — root cause was the un-dismissed KMS dialog above, not a slow backend; `wait_masks_gone(timeout=30000)` is now a safe superset of the old fixed wait for every caller | `flb_wizard_page.py:finish()` |
| **Project-wide 1-TC-per-file convention completed** | Every remaining "combination" test script (parametrize matrices spanning multiple Jira TCs, and files not named `test_njm_<id>.py`) was split project-wide — 189 `test_njm_<id>.py` files, each with exactly one `pytest.mark.jira(...)` marker matching its own filename, 244 tests collected. See `docs/parametrize-pattern.md` for the reversed parametrize-consolidation decision this required | (whole `tests/e2e/` tree) |
